...
MyData <- read.csv(file="left_2fold.csv", sep = ',', header = TRUE)
...
MyPlotDN = volc_DN + geom_text_repel(data=head(MyData, 9), aes(label=gene, size = 3)) + #adding text for the top 22 genes
...
MyData <- read.csv(file="right_2fold.csv", sep = ',', header = TRUE)
...
MyPlotUP = volc_UP + geom_text_repel(data=head(MyData, 12), aes(label=gene, size = 3)) + #adding text for the top 22 genes
    scale_color_manual(values=c("#FFCD00B2", 'chartreuse3',"#CC0C00B2")) + #seting color manually
    geom_hline(yintercept=2, linetype="dashed", color = "red") + #dashed horizontal line
